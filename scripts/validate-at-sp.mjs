// Validação E2E local da porta geo /assistente-tecnico-medico/sp (roda sobre dist/, pós-build).
//
//   node scripts/validate-at-sp.mjs
//
// Sobe um servidor estático que emula os rewrites/redirects do vercel.json e, com o Chromium
// do Playwright, verifica os requisitos críticos da campanha SP:
//   1. 301 do caminho pai /assistente-tecnico-medico -> /
//   2. head da porta SP (title/robots noindex/canonical) + H1/rodapé com São Paulo, CRM/MG intacto
...
//      + UTMs/gclid propagados ao deeplink do Tintim
//   5. home "/" sem regressão (H1 nacional, robots index, sede MG)
...
const TINTIM_SP_TOKEN = "f42174e7-24cc-4583-a56b-9a6161434a15"; // fluxo Tintim dedicado da porta SP
const TINTIM_HOME_TOKEN = "b40002a5-951b-4014-a30a-17af3f592141"; // fluxo Tintim da home
const UTMS = "utm_source=google&utm_medium=cpc&utm_campaign=teste_sp_validacao&utm_term=assistente%20tecnico%20medico&matchtype=e&gclid=TESTGCLID_SP_123";
...
function startServer() {
  return new Promise((resolve) => {
    const srv = createServer(async (req, res) => {
      ...
      let file;
      if (p === "/assistente-tecnico-medico/sp" || p === "/assistente-tecnico-medico/sp/") file = path.join(DIST, "assistente-tecnico-medico-sp.html");
...
// normaliza entradas do dataLayer (Arguments do gtag stub viram {0:...,1:...,2:{...}})
const DL_DUMP = `(window.dataLayer || []).map((e) => {
  if (e && typeof e === "object" && !Array.isArray(e) && "0" in e) return Array.from(e);
  return e;
})`;
...
async function main() {
  ...
  // ── 2+3+4) Porta SP ───────────────────────────────────────────────────────
  console.log("\n[2] Porta SP — head, conteúdo geo e tracking");
  ...
  await page.goto(`${BASE}/assistente-tecnico-medico/sp?${UTMS}`, { waitUntil: "domcontentloaded" });

  check((await page.title()).includes("São Paulo"), "title menciona São Paulo", await page.title());
  ...
  // copy do <main> DEVE ser idêntica à da home EXCETO a seção Atuação (variante SP) —
  // comparada no bloco [4] após normalizar as duas frases da Atuação.
  const spMainText = (await page.textContent("main")).replace(/\s+/g, " ").trim();
  check(spMainText.includes("Acompanhamento pericial em São Paulo e em todo o Brasil"), "Atuação SP: título com São Paulo");
  check(spMainText.includes("em São Paulo, Campinas ou qualquer outra cidade do país."), "Atuação SP: São Paulo/Campinas no texto");
  ...
  if (pv) {
    ...
    check(dl.includes("/assistente-tecnico-medico/sp"), "page_view dl com path da rota", dl.slice(0, 110));
    check(dl.includes("utm_campaign=teste_sp_validacao") && dl.includes("gclid=TESTGCLID_SP_123"), "page_view dl carrega UTMs+gclid");
    const gclidParam = new URL(pv).searchParams.get("gclid");
    check(gclidParam === "TESTGCLID_SP_123", "gclid como param do beacon (atribuição Ads)", gclidParam || "(ausente)");
  }
  ...
  // rola até o fim (dispara GTM/FAB por interação real) e espera o FAB montar, se conseguir
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await page.waitForSelector(".bfab-cta", { timeout: 5000 }).catch(() => {});

  console.log("\n[3] Cliques em todos os botões de WhatsApp da porta SP");
  ...
  for (const b of resultado) {
    const temAw = b.novos.some((e) => Array.isArray(e) && e[0] === "event" && e[1] === "conversion" && e[2] && e[2].send_to === "AW-16690821688/zjEbCOiG7MAcELj05pY-" && e[2].transport_type === "beacon");
    ...
    const temUtm = b.href.includes("utm_campaign=teste_sp_validacao") && b.href.includes("gclid=TESTGCLID_SP_123");
    ...
    check(b.href.includes(TINTIM_SP_TOKEN), `[${b.cta}] deeplink Tintim do fluxo SP`);
  ...
  const wppGa = gaHits.filter((u) => u.includes("whatsapp") || u.includes("en=scroll_depth")).length;
  console.log(`  beacons GA4 observados na rede: ${gaHits.length} (page_view/scroll/etc: ${wppGa} relacionados)`);

  await page.close();

  // ── 5) Home "/" sem regressão ─────────────────────────────────────────────
  console.log("\n[4] Home / — sem regressão");
  ...
  check(homeMainText.includes("Acompanhamento presencial em Goiânia, Brasília, Uberlândia e em todo o Brasil"), "Atuação da home intacta (Goiânia/Brasília/Uberlândia)");
  const spNormalized = spMainText
    .replace("Acompanhamento pericial em São Paulo e em todo o Brasil", "Acompanhamento presencial em Goiânia, Brasília, Uberlândia e em todo o Brasil")
    .replace("em São Paulo, Campinas ou qualquer outra cidade do país.", "em Goiânia, Brasília, Uberlândia e em qualquer comarca do país.");
  check(spNormalized === homeMainText, "copy do <main> idêntica à home (exceto seção Atuação)");
  ...
  const pvHome = gaHome.find((u) => u.includes("en=page_view"));
  check(!!pvHome, "beacon page_view da home segue disparando");
  await home.close();
